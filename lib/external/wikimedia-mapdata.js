...
var MapdataLoader = function (
	 ...
	 * @param {boolean} parsoid whether the page was rendered with Parsoid
	 * @return {Promise<Object>} Resolves to the returned mapdata, or rejects.
	 */
	MapdataLoader.prototype.fetchGroups = function ( groupIds, title, revid, lang, parsoid ) {
		if ( !groupIds.length ) {
		...
		var params = {
			...
			mpdparser: parsoid ? 'parsoid' : 'legacy',
		};
...
var DataManager = function ( wrappers ) {
	 ...
	 * @param {string|false} [lang] Language, used for variants
	 * @param {boolean} parsoid whether the page rendered with Parsoid
	 * @return {Promise<Group[]>} Resolves with a list of expanded Group objects.
	 * @public
	 */
	function loadGroups( groupIds, title, revid, lang, parsoid ) {
